...
function Client(options) {
    ...
    let statsdUrl = process.env.STATSD_URL && url.parse(process.env.STATSD_URL);
    ...
    statsdClient.socket.on('error', function (e) {
        if (e.code === 'ENOTFOUND') {
            logger.error('statsdClient socket error', { statsUrl: process.env.STATSD_URL }, e);
        } else {
            throw e;
        }
    });
    self.metricReporter = new MetricReporter(statsdClient, null);
...
Client.prototype.produceRequest = function (requests, codec) {
    ...
    var produceRequestTime, sendingBufferTime, response;
    ...
    return self._waitMetadata().then(function () {
        produceRequestTime = Timer.start();
            ...
            return Promise.all(_.map(requests, function (topics, leader) {
                ...
                return self.brokerConnections[leader].send(correlationId, buffer, self.options.requiredAcks === 0).then(function (responseBuffer) {
                    ...
                    if (self.options.requiredAcks !== 0) {
                        ...
                        response = self.protocol.read(responseBuffer).ProduceResponse();
                        self.metricReporter.reportTiming('edgebroker.client.produce_request.response', responseTime.getTime());
        ...
        .then(topics => {
            self.metricReporter.reportTiming('edgebroker.client.produce_request.send', sendingBufferTime.getTime());
            return topics;
        })
        .then(_mapTopics)
        .then((topics) => {
            self.metricReporter.reportTiming('edgebroker.client.produce_request', produceRequestTime.getTime());
            return topics;
        });
    });
};
